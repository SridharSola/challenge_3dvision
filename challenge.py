...
def visualize(filename, img_0, img_1, ps_0, ps_1, visualize_steps=False):
    ...
    # Create a darker colormap by reducing brightness
    # TODO: use this to color code the points darker
    # colors = plt.cm.hsv(hues)
    # colors[:, :3] *= 0.7  # Reduce RGB values to make colors darker

    fig, axs = plt.subplots(1, 2, layout="constrained", figsize=(10, 5))
    for ax in axs.flatten():
        ax.axis("off")
    axs[0].imshow(img_0)
    axs[0].scatter(ps_0[:, 0], ps_0[:, 1], c=hues, cmap="hsv", s=0.2, alpha=0.5)
    axs[1].imshow(img_1)
    axs[1].scatter(ps_1[:, 0], ps_1[:, 1], c=hues, cmap="hsv", s=0.2, alpha=0.5)
    plt.savefig(filename, dpi=300)
    if visualize_steps:
        plt.show()


def generate(idx, visualize_steps=False):
    ...
    # Call the main correspondence finding function from solution.py
    from solution import find_pixel_correspondences
    img_0, ps_0, img_1, ps_1 = find_pixel_correspondences(idx[0], idx[1], visualize_steps)
...
if __name__ == "__main__":
    ...
    for i, sample_idx in enumerate(sample_idxs):
        img_0, ps_0, img_1, ps_1 = generate(sample_idx, visualize_steps=args.visualize)
        imgs_0.append(img_0)
        imgs_1.append(img_1)
        ps_0_list.append(ps_0)
        ps_1_list.append(ps_1)
    imgs_0 = np.stack(imgs_0)  # Include stacking in timing
